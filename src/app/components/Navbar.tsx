...
export default function Navbar() {
  ...
  const navItems = [
    {
      label: '요정',
      children: [
        { href: '/fairy', label: '요정의 초상' },
        { href: '/baroque', label: '바로크 요정들' },
  ...
  const groupedHoverIndexes = [0, 1, 6];
  const hasHoveredItem = hoveredIndex !== null && groupedHoverIndexes.includes(hoveredIndex);

  return (
    <>
      <nav
        className={`px-6 bg-white border-b relative z-50 transition-all duration-300 min-h-[64px] ${hasHoveredItem ? 'pb-[96px]' : ''}`}
      >
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-start gap-[64px] text-[18px] font-semibold">
              {navItems.map((item, idx) => (
                <NavGroup
                  ...
                  showGrouped={groupedHoverIndexes.includes(hoveredIndex ?? -1) && groupedHoverIndexes.includes(idx)}
                />
              ))}
            </div>
          )}
