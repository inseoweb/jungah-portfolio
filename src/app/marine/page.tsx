...
import { loc, NEEDS_TRANSLATION, PENDING_TRANSLATION_NODE, type Localized } from '../../lib/i18n';
...
//
// Titles already used in English on the existing site (Dinner 2011,
// Under the Sea, Picturesque, Hug me - 돌아온 탕아) are kept as-is for
// both languages rather than re-translated. Everything else is
// NEEDS_TRANSLATION until a confirmed official English title exists.
const WORKS: MarineWork[] = [
  {
    title: loc('Dinner 2011', 'Dinner 2011'),
    caption: loc('63x63x16(cm), 한지캐스팅,수채,바다쓰레기, 2011', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/1.jpg', alt: 'Dinner 2011', orientation: 'square' }],
  },
  {
    title: loc('빈자리', NEEDS_TRANSLATION),
    caption: loc('73x190(cm), 캔버스에 아크릴릭,바다쓰레기,거울, 2011, 2017', NEEDS_TRANSLATION),
    images: [
  ...
  },
  {
    ...
    caption: loc('162x264(cm), 패널에 아크릴릭, 바다쓰레기, 2022', NEEDS_TRANSLATION),
    images: [
  ...
  {
    title: loc('아름다운 강산', NEEDS_TRANSLATION),
    caption: loc('2022', '2022'),
    images: [{ src: '/images/marine/9.jpg', alt: '아름다운 강산', orientation: 'landscape' }],
  },
  {
    title: loc('바다 귀 귀울여봐요', NEEDS_TRANSLATION),
    caption: loc('30x50(cm), 종이에 수채, 2011', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/10.jpg', alt: '바다 귀 귀울여봐요', orientation: 'landscape' }],
  },
  {
    title: loc('인공파도', NEEDS_TRANSLATION),
    caption: loc('66x100(cm), 패널에 바다쓰레기, 2012', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/11.jpg', alt: '인공파도', orientation: 'landscape' }],
  },
  {
    title: loc('인공파도 2', NEEDS_TRANSLATION),
    caption: loc('80.3x365(cm), 패널에 바다쓰레기, 2012', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/12.jpg', alt: '인공파도 2', orientation: 'landscape' }],
  },
  {
    title: loc('Picturesque', 'Picturesque'),
    caption: loc('71x81(cm), 패널에 유화, 액자, 바다쓰레기, 2018', NEEDS_TRANSLATION),
    images: [
  ...
  {
    title: loc('한없이 무거운', NEEDS_TRANSLATION),
    caption: loc('110x210x80(cm), 납(낚시 납추, 납 지지대), 스테인리스 스틸, 2019', NEEDS_TRANSLATION),
    images: [
  ...
  {
    title: loc('9시 46분', NEEDS_TRANSLATION),
    caption: loc('87x87(cm), 패널에 아크릴릭, 서해 바다쓰레기, 2020', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/18.jpg', alt: '9시 46분', orientation: 'square' }],
  },
  {
    title: loc('한걸음 다가서면 바꿀 수 있어요', NEEDS_TRANSLATION),
    caption: loc('60x110(cm), 렌티큘러, 2020', NEEDS_TRANSLATION),
    images: [
  ...
  {
    title: loc('바다 빼기 바다', NEEDS_TRANSLATION),
    caption: loc('나무에 아크릴릭, 레진, 바다쓰레기, 2021', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/22.jpg', alt: '바다 빼기 바다', orientation: 'portrait' }],
  },
  {
    title: loc('자승자박', NEEDS_TRANSLATION),
    caption: loc('120x80(cm), 패널에 낚시 쓰레기, 2021', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/23.jpg', alt: '자승자박', orientation: 'portrait' }],
  },
  {
    title: loc('궤도 이탈', NEEDS_TRANSLATION),
    caption: loc('가변설치, 유리 부이, 아크릴릭, 바다쓰레기, 2013', NEEDS_TRANSLATION),
    images: [
  ...
  {
    title: loc('해변에 나타난 별자리', NEEDS_TRANSLATION),
    caption: loc('거제 사곡해수욕장, 2023', NEEDS_TRANSLATION),
    images: [{ src: '/images/marine/28.jpg', alt: '해변에 나타난 별자리', orientation: 'landscape' }],
  },
  {
    title: loc('즐거운 추억', NEEDS_TRANSLATION),
    caption: loc('162x130(cm), 패널에 바다쓰레기, 2021', NEEDS_TRANSLATION),
...
const IMAGES: EditorialImage[] = WORKS.flatMap((work) =>
  work.images.map((img) => ({
    ...img,
    caption: {
      ko: work.caption ? `${work.title.ko} · ${work.caption.ko}` : work.title.ko,
      en: work.caption ? `${work.title.en} · ${work.caption.en}` : work.title.en,
    },
  })),
);
